...
ALTER FUNCTION m10_getconfiguracion(integer)
    ...
    -- Function: m10_sendvideonotificacion(integer)

--DROP FUNCTION m10_sendvideonotificacion(integer);

CREATE OR REPLACE FUNCTION m10_sendvideonotificacion ( IN usuario integer)
    RETURNS TABLE(vid_id integer, vid_titulo character varying, vid_descripcion character varying, vid_url character varying, 
                vid_fecha Date, vid_visitas integer, id_cat integer, vid_imagen character varying, usu_id integer) AS
$BODY$
BEGIN
RETURN QUERY
SELECT  * FROM video v
...
$BODY$
    ...
    COST 100
    ROWS 1000;
ALTER FUNCTION m10_sendvideonotificacion(integer)
    OWNER TO postgres;
